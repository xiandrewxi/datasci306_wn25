...
f(2)
```
## Small aside: `drop = TRUE`

Sometimes we get tables, but we actually just want a value.
...
When pulling out values setting the `drop = TRUE` argument strips away the table, if possible:


```{r}
lap_mem[1, 1, drop = TRUE]
```
...
```{r}
group_by(Batting, playerID) |>
  summarize_at(c("H", "X2B", "X3B"), sum) |>
  transmute_at(c("H", "X2B", "X3B"), at_least_bb) |>
  summarize_all(sum)
```
