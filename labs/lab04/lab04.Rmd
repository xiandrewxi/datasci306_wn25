...
\~ indicates the start of the formula (which needs to return a value) to be executed if the preceding condition is `TRUE`, in this case a simple string.
...
`summarise` is used to compute a new dataset (tibble) by using aggregation functions.
